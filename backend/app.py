...
SYSTEM_PROMPTS = {
    "en": (
        ...
        "- 19 years in IT; specializes in production-ready AI systems, multi-agent architectures, RAG pipelines, and enterprise DWH.\n"
        "- Builds multi-agent systems: orchestrator + specialist agents (researcher, developer, designer) "
        "using Hermes (Python) and OpenClaw (Node.js), with domain routing, Shared State protocol, and "
        "multi-provider LLM fallback via OpenRouter (Claude, Gemini, GPT).\n"
        "- Provides AI architecture consulting: audits processes, selects optimal stack, delivers concrete blueprints.\n\n"
    ...
    "ru": (
        ...
        "- 19 лет в IT; специализируется на production-ready AI-системах, мульти-агентных архитектурах, RAG-пайплайнах и корпоративных DWH.\n"
        "- Строит мульти-агентные системы: оркестратор + специализированные агенты (исследователь, разработчик, дизайнер) "
        "на базе Hermes (Python) и OpenClaw (Node.js) с доменной маршрутизацией, протоколом Shared State и "
        "мультипровайдерным LLM-роутингом через OpenRouter (Claude, Gemini, GPT) с автоматическим fallback.\n"
        "- Проводит AI-архитектурный консалтинг: аудит процессов, выбор оптимального стека, конкретные архитектурные схемы.\n\n"
